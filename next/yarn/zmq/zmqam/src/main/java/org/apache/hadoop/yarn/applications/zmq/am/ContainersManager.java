...
import org.apache.hadoop.yarn.applications.zmq.am.ContainersManager.Status;
import org.apache.hadoop.yarn.applications.zmq.am.MyContainer.MyContainerStatus;
...
public class ContainersManager {
  ...
  private ContainerRequest setupContainerAskForRM(String[] nodes, String[] racks) {
    ...
    // generate the endpointList file to workerDir/endpointlist
    private void GenerateEndpointList() {
      try {
        BufferedWriter out = new BufferedWriter(new FileWriter(
            DSConstants.WORKERDIR + File.separator + DSConstants.ENDPOINTLIST));
        for (MyContainer c : serverBackup) {
            out.write(c.id_ + " " + c.container.getNodeId().getHost() + ":" + workerServerPort_ + "\n");
        }
        out.close();
      } catch (Exception e) {
        LOG.error("Failed to generate endpointlist " + e + "\n");
      }
    }
};