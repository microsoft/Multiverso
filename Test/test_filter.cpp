...
class test_filter : public multiverso::SparseFilter<float, int32_t>,
    public ::testing::Test {
 public:
 ...
 protected:
    ...
    bool is_same_content_with_clipping(float* array,
        multiverso::Blob blob, size_t count) {
        for (auto i = 0; i < count; ++i) {
...
TEST_F(test_filter, test_should_compress_all_zero) {
    auto size = 8;
    auto array = new float[size]{near_zero_value, near_zero_value,
...
TEST_F(test_filter, test_should_compress_most_zero_a) {
    auto size = 8;
    auto array = new float[size]{1, 0, 0, 0, 0, 0,
...
TEST_F(test_filter, test_should_not_compress_all_none_zero) {
    auto size = 8;
    auto array = new float[size]{none_zero_value, none_zero_value,
